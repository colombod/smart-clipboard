...
The screen-readiness guard checks macOS static-text `value` as well as control `label`. In the first full run, the older guard stopped empty editor, empty history and General Settings before their audits; those locator failures must not be counted as completed screen audits.

Use an interactive macOS session with Xcode's UI testing prerequisites already satisfied. The script selects local ad-hoc signing for its runner build; it does not install certificates or change privacy permissions, automation permissions or system security settings. If Xcode cannot run the audit, the run remains incomplete; do not bypass that failure or report a clean audit. The [22 September 2026 execution record](../../docs/testing/ACCESSIBILITY-NATIVE.md) is **FAIL**: the final full run executed 16 tests (1 passed, 15 failed, 0 skipped). All fifteen screen audits completed across that run and a focused timeout retry; remaining findings were retained. Native checks verified the window-root and history-limit labels. Developer Tools permission was not changed for the successful access retry and is not an established cause of the earlier launch stalls.
