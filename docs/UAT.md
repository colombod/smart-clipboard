...
Later on 22 September, signed/notarized **0.4.0 build 10** was installed locally as a private bootstrap, preserving preferences/history and selecting the existing 8B local model on the official packaged oMLX 0.7.0.dev2 server. The native Connection test passed. Renewed Screen Recording approval required clearing only the obsolete ad-hoc app entry and adding the installed signed app after user authorization; the warning then cleared. Native capture-to-paste acceptance is still pending. Both 8B and the newly downloaded 32B model pass useful synthetic extraction checks but fail Description/SVG fidelity; [the current evidence](testing/OMLX.md) records those failures. The updater's private installation test and native accessibility acceptance remain open. No 0.4 release is public.
